...
describe('ChainTransferManager', () => {
  bench('single agent chain', async () => {
    ...
    const ctx = {
      taskId: 'bench', description: 'test', priority: 1, status: 'running' as const,
      neededCapabilities: [{ type: 'reasoning' as const, level: 5, confidence: 0.5 }],
      hopHistory: [], currentAgent: '', createdAt: Date.now(),
    };
...
describe('MemoryQueue', () => {
  bench('enqueue/dequeue 10K messages', async () => {
    const q = new MemoryQueue();
    for (let i = 0; i < 10000; i++) {
      await q.enqueue({ id: `${i}`, type: 'test', from: 'a', to: 'b', payload: {}, timestamp: i });
    }
    for (let i = 0; i < 10000; i++) {
      await q.dequeue('b');
    }
...
describe('VectorStore', () => {
  bench('search 10K vectors', async () => {
...
describe('Scheduler', () => {
  bench('submit/dequeue 1000 tasks', async () => {
